...
import cv2
import numpy as np
from src.e2vid.model.model import *
from src.e2vid.utils.inference_utils import CropParameters, EventPreprocessor, IntensityRescaler, ImageFilter, ImageDisplay, \
    ImageWriter, UnsharpMaskFilter
from src.e2vid.utils.inference_utils import upsample_color_image, \
    merge_channels_into_color_image  # for color reconstruction
from src.e2vid.utils.util import robust_min, robust_max
from src.e2vid.utils.timers import CudaTimer, cuda_timers
...
class ImageReconstructor:
    def __init__(self, model, height, width, num_bins, device, augmentation=False, standardization=False):
        ...
        self.initialize(self.height, self.width)
    ...
    def update_reconstruction(self, event_tensor, event_tensor_id=None, stamp=None):
        with torch.no_grad():
            ...
            with CudaTimer('Reconstruction'):
                ...
                # Normalize voxel grid: nonzero elements → mean=0, std=1
                nonzero_ev = (events != 0)
                num_nonzeros = nonzero_ev.sum()
                if num_nonzeros > 0:
                    mean = events.sum() / num_nonzeros
                    stddev = torch.sqrt((events ** 2).sum() / num_nonzeros - mean ** 2)
                    events = nonzero_ev.float() * (events - mean) / stddev
