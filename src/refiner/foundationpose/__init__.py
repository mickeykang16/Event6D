...
from Utils import *
import itertools
from src.refiner.foundationpose.training.predict_pose_refine import *
import yaml

class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/'):
    self.gt_pose = None
  ...
  def track_one(self, rgb, depth, K, iteration, extra={}, index=0):
    ...
    if self.debug>=2:
      extra['vis'] = vis
      if vis is not None:
        imageio.imwrite(f'{self.debug_dir}/{index:04d}_vis_score.png', vis)
    self.pose_last = pose
