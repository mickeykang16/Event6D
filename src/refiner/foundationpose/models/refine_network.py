...
class RefineNetEv(nn.Module):
  def __init__(self, cfg=None, c_in=4, c_in_ev=13, n_view=1):
    super().__init__()
    self.cfg = cfg
    if self.cfg.use_BN:
      norm_layer = nn.BatchNorm2d
    ...
    self.encodeB = nn.Sequential(
      ConvBNReLU(C_in=c_in_ev,C_out=64,kernel_size=7,stride=2, norm_layer=norm_layer),
      ConvBNReLU(C_in=64,C_out=128,kernel_size=3,stride=2, norm_layer=norm_layer),
      ResnetBasicBlock(128,128,bias=True, norm_layer=norm_layer),
      ResnetBasicBlock(128,128,bias=True, norm_layer=norm_layer),
    )
...
class RefineNetEv_v2(nn.Module):
  def __init__(self, cfg=None, c_in=4, c_in_ev=13, n_view=1):
    super().__init__()
    self.cfg = cfg
    if self.cfg.use_BN:
  ...
  def forward(self, A, B, trackB):
    ...
    b_fused = self.fuseB(torch.cat([b, trackB], dim=1))
    # a = x[:bs]
    # b = x[bs:]

    ab = torch.cat((a,b_fused),1).contiguous()
    ab = self.encodeAB(ab)  #(B,C,H,W)
